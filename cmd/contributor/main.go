...
func run(args []string) error {
	...
	if readinessFD, err := strconv.Atoi(os.Getenv("DONATE_CLANKER_READINESS_FD")); err == nil {
		if readiness := os.NewFile(uintptr(readinessFD), "donate-clanker-readiness"); readiness != nil {
			client.Readiness = readiness
			defer readiness.Close()
		}
	}
	return client.Run(ctx, creds, &handler)
}
